...
export class AIService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!apiKey) {
      this.logger.warn(
        'OpenAI API key not configured. AI estimates will not be available.',
      );
    } else {
  ...
  async generateAuditEstimate(
    ...
    if (!this.assistantId) {
      throw new Error(
        'OpenAI Assistant not configured. Please set OPENAI_ASSISTANT_ID environment variable.',
      );
    }
    ...
    try {
      ...
      this.logger.log(
        'AI audit estimate generated successfully using assistant',
      );
      return parsedResponse;
    } catch (error) {
      this.logger.error(
        `Failed to generate AI audit estimate: ${error.message}`,
      );
  ...
  private async createEnhancedAnalysis(
    ...
    const contractDetails = analysis.contractFiles.map((file) => {
      ...
      const name = fileName
        ? fileName.replace('.sol', '').replace('.rs', '')
        : 'Unknown';
      const path = file;
      ...
      if (
        ...
        name.toLowerCase().includes('staking') ||
        ...
        name.toLowerCase().includes('rewards') ||
        name.toLowerCase().includes('incentive')
      ) {
    ...
    const dependencyAnalysis = analysis.dependencies.map((dep) => {
      ...
      if (dep.includes('openzeppelin')) {
        ...
        securityImplications =
          'Lending protocol integration - complex interactions';
        riskLevel = 'high';
      } else if (
        dep.includes('hardhat') ||
        dep.includes('truffle') ||
        dep.includes('foundry')
      ) {
    ...
    if (
      contractDetails.some((c) => c.estimatedPurpose.includes('Governance'))
    ) {
    ...
    const testCoverageAnalysis = {
      coverage: (
        analysis.testFiles > analysis.solidityFiles * 2
          ? 'excellent'
          : analysis.testFiles > analysis.solidityFiles
            ? 'good'
            : analysis.testFiles > analysis.solidityFiles * 0.5
              ? 'fair'
              : 'poor'
      ) as 'poor' | 'fair' | 'good' | 'excellent',
      reasoning: `Found ${analysis.testFiles} test files for ${analysis.solidityFiles} contract files`,
      missingTests: contractDetails
        .filter(
          (c) =>
            !analysis.contractFiles.some((f) =>
              f.includes(c.name.toLowerCase()),
            ),
        )
  ...
  private async useAssistant(
    ...
    try {
      ...
      if (runStatus.status !== 'completed') {
        throw new Error(
          `Assistant run did not complete. Status: ${runStatus.status}`,
        );
      }
    ...
    try {
      ...
      return {
        duration: {
          ...
          unit: 'days' as const,
          reasoning:
            parsed.duration.reasoning ||
            'AI-generated estimate based on enhanced repository analysis',
        },
        resources: {
          seniorAuditors: Math.max(
            0,
            Math.round(parsed.resources.seniorAuditors),
          ),
          juniorAuditors: Math.max(
            0,
            Math.round(parsed.resources.juniorAuditors),
          ),
          reasoning:
            parsed.resources.reasoning ||
            'AI-generated resource allocation based on project complexity',
        },
        cost: {
          ...
          reasoning:
            parsed.cost.reasoning ||
            'AI-generated cost estimate based on market rates and complexity',
        },
        riskFactors: Array.isArray(parsed.riskFactors)
          ? parsed.riskFactors
          : [],
        specialConsiderations: Array.isArray(parsed.specialConsiderations)
