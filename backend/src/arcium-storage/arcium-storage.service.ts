...
export class ArciumStorageService implements OnModuleInit {
  ...
  constructor(private configService: ConfigService) {
    try {
      ...
      const possiblePaths = [
        ...
        path.resolve(
          process.cwd(),
          '../../program/target/idl/bulwark_storage.json',
        ), // From backend to program
        path.resolve(
          process.cwd(),
          '../program/target/idl/bulwark_storage.json',
        ), // Alternative
  ...
  async onModuleInit() {
    if (!this.isEnabled) {
      this.logger.debug(
        'Arcium Storage Service is disabled, skipping MXE public key fetch',
      );
      return;
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (mxePublicKey) {
          ...
          this.logger.debug(
            `MXE x25519 pubkey: ${Buffer.from(mxePublicKey).toString('hex').slice(0, 32)}...`,
          );
          return;
        }
      } catch (error: any) {
  ...
  private encryptCommitHash(commitHash: string): number[] {
    ...
    const encryptedArray: number[] = Array.from(ciphertext as any).map(
      (x: any) => Number(x),
    );

    this.logger.debug(
      `Encrypted commit hash: ${commitHash} -> ${encryptedArray.length} bytes`,
    );
   ...
   */
  async storeAuditResults(data: AuditStorageData): Promise<StorageResult> {
    if (!this.isEnabled) {
      this.logger.warn(
        'Arcium Storage Service is disabled. Cannot store audit results.',
      );
      return {
        success: false,
        message:
          'Arcium Storage Service is not configured. Please set SOLANA_KEYPAIR_PATH or SOLANA_PRIVATE_KEY.',
      };
    }
    ...
    try {
      ...
      if (data.minResources < 0 || data.minResources > 255) {
        throw new Error(
          `Invalid minResources: ${data.minResources}. Must be 0-255.`,
        );
      }
      if (data.maxResources < 0 || data.maxResources > 255) {
        throw new Error(
          `Invalid maxResources: ${data.maxResources}. Must be 0-255.`,
        );
      }
      ...
      const auditRecordPDA = PublicKey.findProgramAddressSync(
        [Buffer.from('audit_record'), reportId.toArrayLike(Buffer, 'le', 8)],
      ...
      this.logger.log(
        `  Team: ${data.minResources}-${data.maxResources} auditors`,
      );
