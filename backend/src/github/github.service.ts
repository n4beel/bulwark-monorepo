...
export class GitHubService {
     ...
     */
    async analyzeRepository(repoPath: string, selectedFiles?: string[]): Promise<RepositoryMetadata['analysis']> {
        try {
            const analysis = {
                totalLines: 0,
                solidityFiles: 0,
            ...
            if (selectedFiles && selectedFiles.length > 0) {
                ...
                contractFiles = selectedFiles.map(filePath => path.join(repoPath, filePath));
                analysis.contractFiles = selectedFiles;
            } else {
                // Use all contract files
                contractFiles = await this.findContractFiles(repoPath);
                analysis.contractFiles = contractFiles.map(file => path.relative(repoPath, file));
            }
            ...
            if (selectedFiles && selectedFiles.length > 0) {
                analysis.totalLines = await this.countLinesOfCodeForFiles(contractFiles);
            } else {
    ...
    private async countLinesOfCodeForFiles(files: string[]): Promise<number> {
        ...
        for (const file of files) {
            try {
                ...
                this.logger.warn(`Could not count lines for file ${file}: ${error.message}`);
            }
        }
    ...
    private async countLinesOfCode(repoPath: string): Promise<number> {
        try {
            ...
            const { stdout: filesOutput } = await execAsync(`find "${repoPath}" -name "*.sol" -o -name "*.rs" -type f`);
    ...
    private async findContractFiles(repoPath: string): Promise<string[]> {
        try {
            ...
            const { stdout } = await execAsync(`find "${repoPath}" -name "*.sol" -o -name "*.rs" -type f`);
            ...
            this.logger.log(`Found ${filteredFiles.length} contract files in ${repoPath}`);
        ...
        for (const config of configFiles) {
            if (fs.existsSync(path.join(repoPath, config.file))) {
                ...
                if (config.file === 'Cargo.toml') {
                    ...
                    const cargoContent = fs.readFileSync(path.join(repoPath, config.file), 'utf8');
    ...
    private async countTestFiles(repoPath: string): Promise<number> {
        try {
            const { stdout } = await execAsync(`find "${repoPath}" -name "*test*" -o -name "*spec*" -type f`);
            const files = stdout.trim().split('\n').filter(line => line.length > 0);
            return files.length;
        } catch (error) {
