...
export class RustAnalyzerService {
  ...
  async isAvailable(): Promise<boolean> {
    try {
      ...
      this.logger.error(
        'Rust analyzer service availability check failed:',
        error,
      );
      return false;
    }
    ...
    try {
      ...
      try {
        ...
        if (result?.repository?.files && result.repository.files.length > 0) {
          ...
          this.logger.debug(
            `Analysis complete for ${filePath}: ${fileResult.function_count} functions, ${fileResult.aggregated.total_arithmetic_ops} operations`,
          );
          return fileResult;
        } else {
        ...
        try {
          ...
          this.logger.warn(
            `Failed to clean up workspace ${workspaceId}:`,
            cleanupError,
          );
        }
      }
  ...
  async analyzeDirectory(
    ...
    try {
      ...
      try {
        ...
        this.logger.debug(
          `Directory analysis complete: ${fileMetrics.length} files analyzed`,
        );
  ...
  convertToStaticAnalysisReport(
    ...
    return {
      ...
      complex_math_operations:
        rustResult.aggregated.total_arithmetic_ops +
        rustResult.aggregated.total_math_functions,
      checked_arithmetic_operations: this.sumArithmeticOperations(
        rustResult,
        'checked',
      ),
      raw_arithmetic_operations: this.sumArithmeticOperations(
        rustResult,
        'raw',
      ),
      ...
      max_cyclomatic_complexity:
        rustResult.aggregated.max_cyclomatic_complexity,
  ...
  private calculateComplexityScore(rustResult: RustAnalysisResult) {
    ...
    const economic = Math.min(
      25,
      (defiPatterns / rustResult.function_count) * 25,
    );
  ...
  private sumArithmeticOperations(
    rustResult: RustAnalysisResult,
    type: 'checked' | 'raw',
  ): number {
  ...
  private countFunctionArithmetic(func: RustFunctionMetrics): number {
    ...
    return (
      ...
      arithmetic.checked_sub +
      arithmetic.checked_mul +
  ...
  private countFunctionMathFunctions(func: RustFunctionMetrics): number {
    ...
    return (
      ...
      math_functions.floor +
      math_functions.ceil +
      math_functions.round +
      math_functions.abs +
      math_functions.min_max
    );
  }
  ...
  private identifyRiskFactors(rustResult: RustAnalysisResult): string[] {
    ...
    if (aggregated.safety_ratio < 0.7) {
      factors.push(
        `Low safety ratio: ${Math.round(aggregated.safety_ratio * 100)}% of operations use safe arithmetic`,
      );
    }

    if (aggregated.avg_cyclomatic_complexity > 10) {
      factors.push(
        `High average cyclomatic complexity: ${aggregated.avg_cyclomatic_complexity.toFixed(1)}`,
      );
    }

    if (aggregated.total_unsafe_ops > 5) {
      factors.push(
        `${aggregated.total_unsafe_ops} potentially unsafe operations detected`,
      );
    }
