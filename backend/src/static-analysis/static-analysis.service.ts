...
export class StaticAnalysisService {
  ...
  async analyzeRustContract(
    ...
    try {
      ...
      try {
        ...
        const { base: rustUrl, warning: rustWarn } = normalizeRustBase(
          process.env.RUST_ANALYZER_URL || 'http://localhost:8080',
        );
        if (rustWarn) this.logger.warn(`[AUG][legacy] ${rustWarn}`);

        // Get timeout from environment variable, default to 5 minutes
        const rustTimeout = parseInt(
          process.env.RUST_ANALYZER_TIMEOUT || '300000',
        );
        ...
        try {
          this.logger.debug(
            `[AUG][legacy] POST ${rustUrl}/augment workspace=${staging.workspaceId} selected=${augmentPayload.selected_files ? augmentPayload.selected_files.length : 'ALL'}`,
          );
          const augResp = await axios.post(
            `${rustUrl}/augment`,
            augmentPayload,
            { timeout: rustTimeout },
          );
          if (augResp.data?.success) {
            ...
            for (const key of overridden) {
              if (key in factors) {
                ...
                this.logger.debug(
                  `Overrode analysisFactors.${key} = ${factors[key]}`,
                );
              }
            }
            ...
            this.logger.log(
              `Rust augmentation applied (overridden=${overridden.join(',') || 'none'}) for ${owner}/${repo}`,
            );
            this.logger.debug(
              `Legacy augmentation factors=${JSON.stringify(factors)}`,
            );
          } else {
            this.logger.warn(
              `Rust augmentation response not successful for ${owner}/${repo}`,
            );
          }
        } catch (augErr) {
          if (
            augErr.code === 'ECONNABORTED' ||
            augErr.message.includes('timeout')
          ) {
            this.logger.warn(
              `Rust augmentation timed out after ${rustTimeout / 1000}s (proceeding without). Consider increasing RUST_ANALYZER_TIMEOUT environment variable for large codebases.`,
            );
          } else {
            this.logger.warn(
              `Rust augmentation failed (proceeding without): ${augErr.message}`,
            );
          }
          const status = augErr?.response?.status;
          if (status === 404) {
            ...
            this.logger.warn(
              `Applied fallback override ${fallbackFactor}=${fallbackValue} due to 404 augment endpoint (legacy path).`,
            );
          }
        }
        ...
        try {
          ...
          this.logger.warn(
            `Failed to cleanup staged workspace: ${cleanupStagingErr.message}`,
          );
        }
      } catch (stagingErr) {
        this.logger.warn(
          `Workspace staging skipped/failed: ${stagingErr.message}`,
        );
      }
  ...
  async getAllReports(
    ...
    try {
      ...
      const reports = await this.staticAnalysisModel
        .find(query)
        .sort({ createdAt: -1 })
        .exec();
  ...
  async getUserReportCount(userId: string): Promise<number> {
    try {
      const count = await this.staticAnalysisModel
  ...
  async getAllReportsCount(): Promise<any | null> {
    try {
      ...
      throw new NotFoundException(
        `Failed to retrieve report count: ${error.message}`,
      );
    }
  }
  ...
  async getReportById(id: string): Promise<any | null> {
    try {
      ...
      this.logger.error(
        `Failed to retrieve report for ${id}: ${error.message}`,
      );
      throw new NotFoundException(
        `Failed to retrieve report for ${id}: ${error.message}`,
      );
  ...
  async associateReportWithUser(
    ...
    try {
      ...
      if (reportObj.userId) {
        throw new Error(
          `Report is already associated with user ${reportObj.userId}`,
        );
      }
      ...
      this.logger.log(
        `Successfully associated report ${reportId} with user ${userId}`,
      );
   ...
   */
  async getReportByRepository(
    repository: string,
  ): Promise<StaticAnalysisReportDocument | null> {
    try {
      const report = await this.staticAnalysisModel
        .findOne({ repository })
        .exec();
      ...
      this.logger.error(
        `Failed to retrieve report for ${repository}: ${error.message}`,
      );
      throw new Error(
        `Failed to retrieve report for ${repository}: ${error.message}`,
      );
  ...
  async analyzeRustContractWithWorkspace(
    ...
    try {
      ...
      try {
        commitHash = await this.githubService.getLatestCommitHash(
          owner,
          repo,
          accessToken,
        );
        ...
        this.logger.warn(
          `Failed to get commit hash: ${error.message}. Continuing without commit information.`,
        );
      }

      // Clone to temp/repos directory (GitHub service already clones to temp/repos)
      repoPath = await this.githubService.cloneRepository(
        repoUrl,
        repoName,
        accessToken,
      );
      ...
      try {
        ...
        this.logger.log(
          `Using analyzeUploadedContract for cloned repository: ${repoName}`,
        );
        ...
        this.logger.log(
          `Successfully completed workspace-based analysis for ${owner}/${repo}`,
        );
        return report;
      } finally {
        ...
        if (repoPath && fs.existsSync(repoPath)) {
          try {
            ...
            this.logger.warn(
              `Failed to cleanup cloned repository: ${cleanupErr.message}`,
            );
          }
        }
      }
      ...
      this.logger.error(
        `Workspace-based analysis failed for ${owner}/${repo}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async analyzeUploadedContract(
    ...
    try {
      ...
      try {
        ...
        try {
          this.logger.debug(
            `[RUST][upload] POST ${rustUrl}/augment workspace=${staging.workspaceId} selected=${augmentPayload.selected_files ? augmentPayload.selected_files.length : 'ALL'}`,
          );
          const augResp = await axios.post(
            `${rustUrl}/augment`,
            augmentPayload,
          ...
          if (augResp.data?.success) {
            ...
            this.logger.log(
              `Rust analysis completed successfully with ${Object.keys(factors).length} factors`,
            );
            this.logger.debug(
          ...
          if (augErr.response) {
            this.logger.warn(
              `Rust analysis error response: ${JSON.stringify(augErr.response.data)}`,
            );
          }
        }
        ...
        try {
          ...
          this.logger.warn(
            `Failed to cleanup staged workspace (uploaded): ${cleanupErr.message}`,
          );
        }
      } catch (stageErr) {
        rustAnalysisError = `Staging failed: ${stageErr.message}`;
        this.logger.warn(
          `Staging skipped for uploaded contract: ${stageErr.message}`,
        );
      }
      ...
      try {
        ...
        this.logger.log(
          `Cleaned up extracted directory and session: ${extractedPath}`,
        );
      } catch (cleanupError) {
        this.logger.warn(
      ...
      this.logger.error(
        `Static analysis failed for uploaded contract: ${error.message}`,
      );
      ...
      try {
        ...
        this.uploadsService.removeUploadSession(extractedPath);
        this.logger.log(
          `Cleaned up extracted directory and session after error: ${extractedPath}`,
        );
      } catch (cleanupError) {
        this.logger.warn(
          `Failed to cleanup extracted directory and session after error: ${cleanupError.message}`,
        );
      }
  ...
  private async analyzeRustFile(
    ...
    const { totalComplexity, maxComplexity } =
      this.calculateCyclomaticComplexity(content, numFunctions);
    ...
    return {
      ...
      cpiUsage: (
        content.match(/token::|invoke|invoke_signed|CpiContext/g) || []
      ).length,
      crossProgramInvocation: [],
      tokenTransfers: (content.match(/transfer|mint|burn/g) || []).length,
      complexMathOperations: await this.getAccurateComplexMathOperations(
        content,
        filePath,
      ),
      timeDependentLogic: (
        content.match(
          /Clock::get|unix_timestamp|timestamp|last_updated|created_at/gi,
        ) || []
      ).length,
      defiPatterns,
      economicRiskFactors,
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (commentStartIndex !== -1) {
        ...
        if (commentEndIndex !== -1) {
          ...
          const hasCodeAfter =
            codeAfterComment.length > 0 && !codeAfterComment.startsWith('//');
  ...
  private detectFramework(repoPath: string): string {
    try {
      ...
      if (fs.existsSync(cargoTomlPath)) {
        ...
        this.logger.log(
          `Cargo.toml contents preview: ${cargoToml.substring(0, 500)}...`,
        );

        // Check for Anchor framework
        if (
          cargoToml.includes('anchor-lang') ||
          cargoToml.includes('anchor-spl')
        ) {
        ...
        if (
          cargoToml.includes('mpl-token-metadata') ||
          cargoToml.includes('metaplex')
        ) {
        ...
        if (
          cargoToml.includes('solana-program') ||
          cargoToml.includes('solana-sdk')
        ) {
        ...
        if (
          cargoToml.includes('spl-token') ||
          cargoToml.includes('spl-associated-token-account')
        ) {
          this.logger.log(
            'Detected Native Solana framework from SPL dependencies',
          );
          return 'native';
        }

        this.logger.log(
          'No framework detected in Cargo.toml, checking other indicators...',
        );
      } else {
      ...
      if (fs.existsSync(srcPath)) {
        ...
        if (hasLibRs || hasMainRs) {
          ...
          if (fs.existsSync(mainFilePath)) {
            ...
            if (
              mainFileContent.includes('anchor_lang') ||
            ...
            if (
              mainFileContent.includes('mpl_token_metadata') ||
            ...
            if (
              mainFileContent.includes('solana_program') ||
      ...
      if (fs.existsSync(workspaceCargoPath)) {
        ...
        for (const program of programs) {
          const programCargoPath = path.join(
            workspaceCargoPath,
            program,
            'Cargo.toml',
          );
          if (fs.existsSync(programCargoPath)) {
            this.logger.log(
              `Checking program Cargo.toml at: ${programCargoPath}`,
            );
            const programCargo = fs.readFileSync(programCargoPath, 'utf-8');
            this.logger.log(
              `Program Cargo.toml contents preview: ${programCargo.substring(0, 300)}...`,
            );

            if (programCargo.includes('anchor-lang')) {
              this.logger.log(
                'Detected Anchor framework from program Cargo.toml',
              );
              return 'anchor';
            }
            if (programCargo.includes('mpl-token-metadata')) {
              this.logger.log(
                'Detected Metaplex framework from program Cargo.toml',
              );
              return 'metaplex';
            }
            if (programCargo.includes('solana-program')) {
              this.logger.log(
                'Detected Native Solana framework from program Cargo.toml',
              );
              return 'native';
            }
          }
        ...
        this.logger.log(
          `Programs directory not found at: ${workspaceCargoPath}`,
        );
      }
      ...
      if (fs.existsSync(rootCargoPath)) {
        ...
        if (rootCargo.includes('[workspace]')) {
          this.logger.log(
            'Found workspace Cargo.toml, checking workspace members...',
          );
          ...
          if (workspaceMatch) {
            ...
            const members =
              membersStr.match(/"([^"]+)"/g)?.map((m) => m.replace(/"/g, '')) ||
              [];
            ...
            for (const member of members) {
              ...
              if (fs.existsSync(memberCargoPath)) {
                ...
                if (memberCargo.includes('anchor-lang')) {
                  this.logger.log(
                    `Detected Anchor framework from workspace member: ${member}`,
                  );
                  return 'anchor';
                }
                if (memberCargo.includes('solana-program')) {
                  this.logger.log(
                    `Detected Native Solana framework from workspace member: ${member}`,
                  );
                  return 'native';
                }
              }
  ...
  private calculateCyclomaticComplexity(
    content: string,
    numFunctions: number,
  ): { totalComplexity: number; maxComplexity: number } {
    ...
    if (functionStarts.length === 0) {
      ...
      const avgComplexityPerFunction = Math.max(
        1,
        Math.ceil(totalDecisionPoints / numFunctions),
      );
      return {
        totalComplexity: avgComplexityPerFunction * numFunctions,
        maxComplexity: Math.min(
          avgComplexityPerFunction * 2,
          totalDecisionPoints,
        ),
      };
    }
    ...
    for (let i = 0; i < functionStarts.length; i++) {
      ...
      const end =
        i < functionStarts.length - 1 ? functionStarts[i + 1] : content.length;
      const functionBody = content.substring(start, end);

      const functionComplexity = Math.max(
        1,
        this.countDecisionPoints(functionBody),
      );
  ...
  private countComplexMathOperations(content: string): number {
    ...
    for (const group of patternGroups) {
      for (const pattern of group.patterns) {
        ...
        while ((match = pattern.exec(cleanedContent)) !== null) {
          ...
          const line = cleanedContent.slice(
            lineStart,
    ...
    if (mathOperationsCount > 500) {
      console.warn(
        `Math operations count (${mathOperationsCount}) seems extremely high, potential pattern overcounting`,
      );
    }
   ...
   */
  private async getAccurateComplexMathOperations(
    content: string,
    filePath?: string,
  ): Promise<number> {
    try {
      ...
      if (filePath && (await this.rustAnalyzerService.isAvailable())) {
        ...
        return (
          result.aggregated.total_arithmetic_ops +
          result.aggregated.total_math_functions
        );
      }
    } catch (error) {
      this.logger.warn(
        `Rust analyzer failed, falling back to regex: ${error.message}`,
      );
    }
  ...
  async exportReportsToCSV(
    ...
    this.logger.log(
      `Exporting reports to CSV - IDs: ${reportIds?.length || 'all'}, Factors: ${factors?.length || 'all'}`,
    );
    ...
    try {
      ...
      if (reportIds && reportIds.length > 0) {
        reports = await this.staticAnalysisModel
          .find({
            _id: { $in: reportIds },
          })
          .exec();
      ...
      const factorsToInclude =
        factors && factors.length > 0 ? factors : this.getDefaultFactors();

      // Step 4: Generate CSV content with new layout (factors as rows, reports as columns)
      const csvContent = this.generateFactorRowsCSV(
        reports,
        factorsToInclude,
        availableFactors,
      );
      ...
      this.logger.log(
        `CSV export completed: ${factorsToInclude.length} factors across ${reports.length} reports`,
      );
      ...
      basic: {
        ...
        factors: {
          ...
          language: {
            name: 'Language',
            type: 'string',
            description: 'Programming language (e.g., rust)',
          },
          framework: {
            name: 'Framework',
            type: 'string',
            description: 'Framework used (e.g., anchor, unknown)',
          },
          createdAt: {
            name: 'Created At',
            type: 'date',
      ...
      scores: {
        ...
        factors: {
          ...
          'scores.security.score': {
            name: 'Security Score',
            type: 'number',
            description: 'Overall security complexity score (0-100)',
          },
          'scores.systemic.score': {
            name: 'Systemic Score',
            type: 'number',
            description:
              'Overall systemic & integration complexity score (0-100)',
          },
          'scores.economic.score': {
            name: 'Economic Score',
            type: 'number',
            description:
              'Overall economic & functional complexity score (0-100)',
          },

          // Structural score details
          'scores.structural.details.totalLinesOfCode': {
            name: 'Structural - Total Lines Of Code',
            type: 'number',
            description: 'Lines of code from structural analysis',
          },
          'scores.structural.details.numContracts': {
            name: 'Structural - Number Of Contracts',
            type: 'number',
            description: 'Number of contracts from structural analysis',
          },
          'scores.structural.details.numFunctions': {
            name: 'Structural - Number Of Functions',
            type: 'number',
            description: 'Number of functions from structural analysis',
          },
          'scores.structural.details.numStateVariables': {
            name: 'Structural - State Variables',
            type: 'number',
            description: 'Number of state variables from structural analysis',
          },
          'scores.structural.details.avgCyclomaticComplexity': {
            name: 'Structural - Avg Cyclomatic Complexity',
            type: 'number',
            description:
              'Average cyclomatic complexity from structural analysis',
          },
          'scores.structural.details.maxCyclomaticComplexity': {
            name: 'Structural - Max Cyclomatic Complexity',
            type: 'number',
            description:
              'Maximum cyclomatic complexity from structural analysis',
          },
          'scores.structural.details.inheritanceDepth': {
            name: 'Structural - Inheritance Depth',
            type: 'number',
            description: 'Inheritance depth from structural analysis',
          },

          // Security score details
          'scores.security.details.lowLevelOperations.assemblyBlocks': {
            name: 'Security - Assembly Blocks',
            type: 'number',
            description: 'Number of assembly blocks',
          },
          'scores.security.details.lowLevelOperations.delegateCalls': {
            name: 'Security - Delegate Calls',
            type: 'number',
            description: 'Number of delegate calls',
          },
          'scores.security.details.lowLevelOperations.rawCalls': {
            name: 'Security - Raw Calls',
            type: 'number',
            description: 'Number of raw calls',
          },
          'scores.security.details.securityCriticalFeatures.payableFunctions': {
            name: 'Security - Payable Functions',
            type: 'number',
            description: 'Number of payable functions',
          },
          'scores.security.details.securityCriticalFeatures.txOriginUsage': {
            name: 'Security - TX Origin Usage',
            type: 'number',
          ...
          'scores.systemic.details.standardInteractions.erc20Interactions': {
            ...
            type: 'boolean',
            description: 'Whether contract interacts with ERC20 tokens',
          },
          'scores.systemic.details.standardInteractions.erc721Interactions': {
            name: 'Systemic - ERC721 Interactions',
            type: 'boolean',
            description: 'Whether contract interacts with ERC721 tokens',
          },
          'scores.systemic.details.standardInteractions.erc1155Interactions': {
            name: 'Systemic - ERC1155 Interactions',
            type: 'boolean',
            description: 'Whether contract interacts with ERC1155 tokens',
          },
          'scores.systemic.details.oracleUsage': {
            name: 'Systemic - Oracle Usage',
            type: 'array',
            description: 'Oracle usage details',
          },
          'scores.systemic.details.accessControlPattern.type': {
            name: 'Systemic - Access Control Type',
            type: 'string',
            description: 'Type of access control pattern',
          },
          'scores.systemic.details.accessControlPattern.complexity': {
            name: 'Systemic - Access Control Complexity',
            type: 'string',
            description: 'Complexity of access control pattern',
          },

          // Economic score details
          'scores.economic.details.financialPrimitives.isAMM': {
            name: 'Economic - Is AMM',
            type: 'boolean',
            description: 'Whether contract is an AMM',
          },
          'scores.economic.details.financialPrimitives.isLendingProtocol': {
            name: 'Economic - Is Lending Protocol',
            type: 'boolean',
            description: 'Whether contract is a lending protocol',
          },
          'scores.economic.details.financialPrimitives.isVestingContract': {
            name: 'Economic - Is Vesting Contract',
            type: 'boolean',
            description: 'Whether contract is a vesting contract',
          },
          'scores.economic.details.financialPrimitives.defiPatterns': {
            name: 'Economic - DeFi Patterns',
            type: 'array',
            description: 'Detected DeFi patterns',
          },
          'scores.economic.details.tokenomics.tokenTransfers': {
            name: 'Economic - Token Transfers',
            type: 'number',
            description: 'Number of token transfers',
          },
          'scores.economic.details.tokenomics.complexMathOperations': {
            name: 'Economic - Complex Math Operations',
            type: 'number',
            description: 'Number of complex math operations',
          },
          'scores.economic.details.tokenomics.timeDependentLogic': {
            name: 'Economic - Time Dependent Logic',
            type: 'boolean',
            description: 'Whether contract has time-dependent logic',
          },
          'scores.economic.details.economicRiskFactors': {
            name: 'Economic - Risk Factors',
            type: 'array',
            description: 'Economic risk factors',
          },
        },
      },
      ...
      analysisFactors: {
        ...
        factors: {
          ...
          'analysisFactors.totalLinesOfCode': {
            ...
            type: 'number',
            description:
              'Total number of code lines (excluding comments and empty lines)',
          },
          'analysisFactors.numPrograms': {
            name: 'Number of Programs',
            type: 'number',
            description: 'Number of Solana programs defined',
          },
          ...
          'analysisFactors.numStateVariables': {
            name: 'State Variables',
            type: 'number',
            description: 'Number of state variables in account structs',
          },
          ...
          'analysisFactors.compositionDepth': {
            ...
            type: 'number',
            description: 'Maximum nesting depth in code blocks',
          },
          ...
          'analysisFactors.functionVisibility.public': {
            ...
            type: 'number',
            description: 'Number of public functions',
          },
          'analysisFactors.functionVisibility.private': {
            name: 'Private Functions',
            type: 'number',
            description: 'Number of private functions',
          },
          'analysisFactors.functionVisibility.internal': {
            name: 'Internal Functions',
            type: 'number',
            description: 'Number of internal functions',
          },
          'analysisFactors.viewFunctions': {
            name: 'View Functions',
            type: 'number',
            description: 'Number of view functions',
          },
          'analysisFactors.pureFunctions': {
            name: 'Pure Functions',
            type: 'number',
            description: 'Number of pure functions',
          },

          // Security metrics
          'analysisFactors.integerOverflowRisks': {
            name: 'Integer Overflow Risks',
            type: 'number',
            description: 'Number of potential integer overflow risks',
          },
          'analysisFactors.accessControlIssues': {
            name: 'Access Control Issues',
            type: 'number',
            description: 'Number of access control issues',
          },
          'analysisFactors.inputValidationIssues': {
            name: 'Input Validation Issues',
            type: 'number',
            description: 'Number of input validation issues',
          },
          'analysisFactors.unsafeCodeBlocks': {
            name: 'Unsafe Code Blocks',
            type: 'number',
            description: 'Number of unsafe Rust code blocks',
          },
          'analysisFactors.panicUsage': {
            name: 'Panic Usage',
            type: 'number',
            description: 'Usage of panic! macro',
          },
          'analysisFactors.unwrapUsage': {
            name: 'Unwrap Usage',
            type: 'number',
            description: 'Usage of .unwrap() method',
          },
          'analysisFactors.expectUsage': {
            name: 'Expect Usage',
            type: 'number',
            description: 'Usage of .expect() method',
          },
          'analysisFactors.matchWithoutDefault': {
            name: 'Match Without Default',
            type: 'number',
            description: 'Number of match expressions without default arm',
          },
          'analysisFactors.arrayBoundsChecks': {
            name: 'Array Bounds Checks',
            type: 'number',
            description: 'Number of array bounds checks',
          },
          'analysisFactors.memorySafetyIssues': {
            name: 'Memory Safety Issues',
            type: 'number',
            description: 'Number of memory safety issues',
          },

          // Integration metrics
          'analysisFactors.externalProgramCalls': {
            name: 'External Program Calls',
            type: 'number',
            description: 'Number of calls to external Solana programs',
          },
          'analysisFactors.uniqueExternalCalls': {
            name: 'Unique External Calls',
            type: 'number',
            description: 'Number of distinct external programs called',
          },
          'analysisFactors.knownProtocolInteractions': {
            name: 'Known Protocol Interactions',
            type: 'array',
            description: 'List of known protocol interactions',
          },
          'analysisFactors.standardLibraryUsage': {
            name: 'Standard Library Usage',
            type: 'array',
            description: 'List of standard libraries used',
          },
          'analysisFactors.oracleUsage': {
            name: 'Oracle Usage',
            type: 'array',
            description: 'List of oracle integrations',
          },
          'analysisFactors.cpiUsage': {
            name: 'CPI Usage',
            type: 'number',
            description: 'Cross-Program Invocation usage count',
          },
          'analysisFactors.crossProgramInvocation': {
            name: 'Cross Program Invocations',
            type: 'array',
            description: 'Detailed CPI call information',
          },
          ...
          'analysisFactors.accessControlPatterns.ownable': {
            ...
            type: 'number',
            description: 'Usage of ownable access control pattern',
          },
          'analysisFactors.accessControlPatterns.roleBased': {
            name: 'Role Based Pattern',
            type: 'number',
            description: 'Usage of role-based access control pattern',
          },
          'analysisFactors.accessControlPatterns.custom': {
            name: 'Custom Access Control',
            type: 'number',
            description: 'Usage of custom access control patterns',
          },
          ...
          'analysisFactors.tokenTransfers': {
            ...
            type: 'number',
            description: 'Number of token transfer operations',
          },
          'analysisFactors.complexMathOperations': {
            name: 'Complex Math Operations',
            type: 'number',
            description: 'Number of complex mathematical operations',
          },
          'analysisFactors.timeDependentLogic': {
            name: 'Time-Dependent Logic',
            type: 'number',
            description: 'Number of time-dependent logic instances',
          },
          'analysisFactors.defiPatterns': {
            name: 'DeFi Patterns',
            type: 'array',
            description: 'List of detected DeFi patterns',
          },
          'analysisFactors.economicRiskFactors': {
            name: 'Economic Risk Factors',
            type: 'array',
            description: 'List of economic risk factors',
          },

          // DeFi Patterns Aggregated
          defiPatterns_total_count: {
            name: 'DeFi Patterns - Total Count',
            type: 'number',
            description: 'Total number of DeFi pattern instances detected',
          },
          defiPatterns_amm_count: {
            name: 'DeFi Patterns - AMM Count',
            type: 'number',
            description: 'Number of AMM (Automated Market Maker) patterns',
          },
          defiPatterns_lending_count: {
            name: 'DeFi Patterns - Lending Count',
            type: 'number',
            description: 'Number of lending protocol patterns',
          },
          defiPatterns_vesting_count: {
            name: 'DeFi Patterns - Vesting Count',
            type: 'number',
            description: 'Number of vesting contract patterns',
          },
          defiPatterns_unique_count: {
            name: 'DeFi Patterns - Unique Types',
            type: 'number',
            description: 'Number of unique DeFi pattern types',
          },
          defiPatterns_types: {
            name: 'DeFi Patterns - Type List',
            type: 'string',
            description: 'Semicolon-separated list of all DeFi pattern types',
          },

          // Economic Risk Factors Aggregated
          economicRiskFactors_total_count: {
            name: 'Economic Risks - Total Count',
            type: 'number',
            description: 'Total number of economic risk factors detected',
          },
          economicRiskFactors_overflow_count: {
            name: 'Economic Risks - Overflow Count',
            type: 'number',
            description: 'Number of integer overflow risks',
          },
          economicRiskFactors_divisionByZero_count: {
            name: 'Economic Risks - Division By Zero Count',
            type: 'number',
            description: 'Number of division by zero risks',
          },
          economicRiskFactors_precisionLoss_count: {
            name: 'Economic Risks - Precision Loss Count',
            type: 'number',
            description: 'Number of precision loss risks',
          },
          economicRiskFactors_severities: {
            name: 'Economic Risks - Severities',
            type: 'string',
            description: 'Semicolon-separated list of severity levels',
          },

          // Program Derives Aggregated
          programDerives_count: {
            name: 'Program Derives - Total Count',
            type: 'number',
            description: 'Total number of derive macros used',
          },
          programDerives_unique_count: {
            name: 'Program Derives - Unique Count',
            type: 'number',
            description: 'Number of unique derive macros',
          },
          programDerives_list: {
            name: 'Program Derives - List',
            type: 'string',
            description: 'Semicolon-separated list of unique derive macros',
          },

          // Standard Library Usage Aggregated
          standardLibraryUsage_count: {
            name: 'Standard Libraries - Count',
            type: 'number',
            description: 'Number of standard libraries used',
          },
          standardLibraryUsage_list: {
            name: 'Standard Libraries - List',
            type: 'string',
            description: 'Semicolon-separated list of standard libraries',
          },

          // Cross Program Invocation
          crossProgramInvocation_count: {
            name: 'Cross Program Invocations - Count',
            type: 'number',
            description: 'Number of cross-program invocations',
          },
          ...
          'analysisFactors.anchorSpecificFeatures.accountValidation': {
            ...
            type: 'number',
            description: 'Number of account validation patterns',
          },
          'analysisFactors.anchorSpecificFeatures.constraintUsage': {
            name: 'Constraint Usage',
            type: 'number',
            description: 'Number of Anchor constraints used',
          },
          'analysisFactors.anchorSpecificFeatures.instructionHandlers': {
            name: 'Instruction Handlers',
            type: 'number',
            description: 'Number of instruction handler functions',
          },
          'analysisFactors.anchorSpecificFeatures.programDerives': {
            name: 'Program Derives',
            type: 'array',
            description: 'List of derive macros used',
          },
          'analysisFactors.anchorSpecificFeatures.accountTypes': {
            name: 'Account Types',
            type: 'number',
            description: 'Number of different account types used',
          },
          'analysisFactors.anchorSpecificFeatures.seedsUsage': {
            name: 'Seeds Usage',
            type: 'number',
            description: 'Usage of PDA seeds',
          },
          'analysisFactors.anchorSpecificFeatures.bumpUsage': {
            name: 'Bump Usage',
            type: 'number',
            description: 'Usage of bump seeds',
          },
          'analysisFactors.anchorSpecificFeatures.signerChecks': {
            name: 'Signer Checks',
            type: 'number',
            description: 'Number of signer validation checks',
          },
          'analysisFactors.anchorSpecificFeatures.ownerChecks': {
            name: 'Owner Checks',
            type: 'number',
            description: 'Number of owner validation checks',
          },
          'analysisFactors.anchorSpecificFeatures.spaceAllocation': {
            name: 'Space Allocation',
            type: 'number',
            description: 'Account space allocation instances',
          },
          'analysisFactors.anchorSpecificFeatures.rentExemption': {
            name: 'Rent Exemption',
            type: 'number',
            description: 'Rent exemption handling instances',
          },
        },
      },
      ...
      performance: {
        ...
        factors: {
          ...
          'performance.memoryUsage': {
            ...
            type: 'number',
            description: 'Memory consumed during analysis',
          },
        },
      },
    };
  ...
  private generateFactorRowsCSV(
    ...
    this.logger.log(
      `Generating factor-rows CSV for ${factorsToInclude.length} factors across ${reports.length} reports`,
    );
    ...
    const headerRow = [
      ...
      ...reports.map((report) => {
        ...
        return (
          reportObj.repository ||
          `Report-${reportObj._id?.toString().slice(-6)}`
        );
      }),
    ];
    ...
    this.logger.log(
      `Generated factor-rows CSV with ${factorsToInclude.length} factor rows and ${reports.length} report columns`,
    );
    return csvRows.join('\n');
  }
  ...
  private getFactorInfo(
    ...
    for (const [categoryKey, categoryData] of Object.entries(
      ...
      if (category.factors && category.factors[factorPath]) {
        return {
          name:
            category.factors[factorPath].name ||
            this.createFriendlyName(factorPath),
  ...
  private getFactorValue(reportObj: any, factorPath: string): any {
    ...
    if (
      factorPath.includes('_count') ||
      factorPath.includes('_types') ||
      factorPath.includes('_list') ||
      factorPath.includes('_severities')
    ) {
  ...
  private extractArraySummary(reportObj: any, factor: string): any {
    ...
    if (factor === 'oracleUsage_count') {
      const oracles =
        this.getNestedValue(reportObj, 'analysisFactors.oracleUsage') || [];
    ...
    if (factor === 'oracleUsage_types') {
      ...
      const types = oracles
        .map((o: any) => o.oracle || o.type || o.name)
        .filter(Boolean);
      return types.join('; ');
    }
    ...
    if (factor === 'defiPatterns_total_count') {
      const patterns =
        this.getNestedValue(reportObj, 'analysisFactors.defiPatterns') || [];
    ...
    if (factor === 'defiPatterns_unique_count') {
      ...
      const uniqueTypes = new Set(
        patterns.map((p: any) => p.type).filter(Boolean),
      );
    ...
    if (factor === 'economicRiskFactors_total_count') {
      const risks =
        this.getNestedValue(reportObj, 'analysisFactors.economicRiskFactors') ||
        [];
    ...
    if (factor === 'economicRiskFactors_overflow_count') {
      ...
      return risks.filter(
        (r: any) => r.type && r.type.toLowerCase().includes('overflow'),
      ).length;
    }
    if (factor === 'economicRiskFactors_divisionByZero_count') {
      ...
      if (!Array.isArray(risks)) return 0;
      return risks.filter(
        (r: any) => r.type && r.type.toLowerCase().includes('division'),
      ).length;
    }
    if (factor === 'economicRiskFactors_precisionLoss_count') {
      ...
      if (!Array.isArray(risks)) return 0;
      return risks.filter(
        (r: any) => r.type && r.type.toLowerCase().includes('precision'),
      ).length;
    }
  ...
  private inferCategory(factorPath: string): string {
    if (
      factorPath.startsWith('scores.structural') ||
      factorPath.includes('LinesOfCode') ||
      factorPath.includes('Functions') ||
      factorPath.includes('Complexity')
    ) {
      return 'Structural';
    }
    if (
      factorPath.startsWith('scores.semantic') ||
      factorPath.startsWith('scores.security') ||
      factorPath.includes('unsafe') ||
      factorPath.includes('panic') ||
      factorPath.includes('Security') ||
      factorPath.includes('unwrap') ||
      factorPath.includes('accessControl')
    ) {
      return 'Security';
    }
    if (
      factorPath.startsWith('scores.systemic') ||
      factorPath.includes('cpi') ||
      factorPath.includes('external') ||
      factorPath.includes('oracle') ||
      factorPath.includes('standardLibrary') ||
      factorPath.includes('knownProtocol') ||
      factorPath.includes('crossProgram')
    ) {
      return 'Integration';
    }
    if (
      factorPath.startsWith('scores.economic') ||
      factorPath.includes('token') ||
      factorPath.includes('defi') ||
      factorPath.includes('economic') ||
      factorPath.includes('Risk') ||
      factorPath.includes('vesting') ||
      factorPath.includes('lending') ||
      factorPath.includes('amm')
    ) {
      return 'Economic';
    }
    if (
      factorPath.includes('anchor') ||
      factorPath.includes('programDerives') ||
      factorPath.includes('instruction') ||
      factorPath.includes('seeds') ||
      factorPath.includes('signer') ||
      factorPath.includes('owner')
    ) {
      return 'Anchor';
    }
    if (
      factorPath.includes('performance') ||
      factorPath.includes('analysisTime') ||
      factorPath.includes('memoryUsage')
    ) {
      return 'Performance';
    }
    if (
      factorPath === 'repository' ||
      factorPath === 'repositoryUrl' ||
      factorPath === 'language' ||
      factorPath === 'framework'
    ) {
   ...
   */
  private async cleanupExtractedDirectory(
    extractedPath: string,
  ): Promise<void> {
    try {
      ...
      this.logger.warn(
        `Failed to cleanup extracted directory: ${error.message}`,
      );
      throw error;
    }
  }
