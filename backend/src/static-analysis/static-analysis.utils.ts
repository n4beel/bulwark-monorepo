...
import {
  AiAnalysisResults,
  CodeMetrics,
} from 'src/ai-analysis/ai-analysis.service';
...
export class StaticAnalysisUtils {
  ...
  calculateTotalScore(
    staticAnalysisScores: any,
    aiAnalysisFactors: CodeMetrics,
    repoData: any,
  ): { scores: any; report: any } {
    const structuralScore =
      staticAnalysisScores.structural['total_statement_count'] * 0.25 +
      staticAnalysisScores.structural[
        'number_of_functions/instructions_handlers'
      ] *
        0.25 +
      staticAnalysisScores.structural['cyclomatic_complexity_&_control_flow'] *
        0.2 +
      staticAnalysisScores.structural[
    ...
    const securityScore =
      staticAnalysisScores.security['access_controlled_handlers'] * 0.2 +
      ...
      staticAnalysisScores.security['arithmetic_operations'] * 0.1 +
      staticAnalysisScores.security['priviliged_roles_& _admin_actions'] * 0.1 +
      staticAnalysisScores.security['unsafe/low_level_usage'] * 0.1 +
      staticAnalysisScores.security['error_handling_footprint'] * 0.05;
    const systemicScore =
      staticAnalysisScores.systemic['upgradability_and_governance_control'] *
        0.2 +
      staticAnalysisScores.systemic['external_integration_&_oracles'] * 0.3 +
      staticAnalysisScores.systemic[
        'composability_and_inter_program_complexity'
      ] *
        0.2 +
      staticAnalysisScores.systemic['denial_of_service_&_resource_limits'] *
        0.15 +
      staticAnalysisScores.systemic['operational_security_factors'] * 0.15;
    const economicScore =
      staticAnalysisScores.economic['number_of_asset_&_asset_types'] * 0.5 +
      staticAnalysisScores.economic['invariants_&_risk_parameters'] * 0.5;
    const totalScore =
      structuralScore * 0.2 +
      securityScore * 0.3 +
      systemicScore * 0.3 +
    ...
    const report = this.calculateResult(aiAnalysisFactors, {
      totalScore,
      ...repoData,
    });
  ...
  calculateResult(aiAnalysisFactors: CodeMetrics, repoData: any): any {
    return {
      ...
      hotspots: {
        totalCount:
          aiAnalysisFactors?.highRiskHotspots?.length +
            aiAnalysisFactors?.mediumRiskHotspots?.length +
            aiAnalysisFactors?.findings?.length || 0,
  ...
  estimatedAuditEffort(totalScore: number): any {
    ...
    const sortedData = [...data].sort(
      (a, b) => a.medianComplexity - b.medianComplexity,
    );

    let lower: {
      medianAuditTimeline: number;
      skewedAuditTimeline: number;
    ...
    if (totalScore >= maxComplexity) {
      ...
      for (let i = 0; i < sortedData.length - 1; i++) {
        if (
          sortedData[i].medianComplexity < totalScore &&
          sortedData[i + 1].medianComplexity > totalScore
        ) {
          lower = sortedData[i];
    ...
    if (upper && lower) {
      const slope =
        (totalScore - lower.medianComplexity) /
        (upper.medianComplexity - lower.medianComplexity);
      medianDays = Math.round(
        lower.medianAuditTimeline +
          slope * (upper.medianAuditTimeline - lower.medianAuditTimeline),
      );
      skewedDays = Math.round(
        lower.skewedAuditTimeline +
          slope * (upper.skewedAuditTimeline - lower.skewedAuditTimeline),
      );
    } else if (!upper && lower) {
      medianDays = Math.round(
        lower.medianAuditTimeline +
          SLOPE_ABOVE_MAX * (totalScore - lower.medianComplexity),
      );
      skewedDays = Math.round(
        medianDays + (lower.skewedAuditTimeline - lower.medianAuditTimeline),
      );
    } else if (!lower && upper) {
      medianDays = Math.round(
        Math.max(
          MIN_MEDIAN_DAYS,
          upper.medianAuditTimeline -
            SLOPE_BELOW_MIN * (upper.medianComplexity - totalScore),
        ),
      );
      skewedDays = Math.round(
        medianDays + (upper.skewedAuditTimeline - upper.medianAuditTimeline),
      );
    }
    ...
    const calculateCost = (days, isLowBudget) => {
      return Math.round(
        (days / DAYS_PER_WEEK) *
          (isLowBudget ? COST_BUDGET_LOW : COST_BUDGET_HIGH),
      );
    };
    ...
    console.log('===============================================');
    console.log(
      `timeline for 2 auditors: ${medianDays} days - ${skewedDays} days`,
    );
    console.log(
      `cost for 2 auditors budget: ${cost2Low * 2} - ${cost2High * 2}`,
    );
    console.log('===============================================');
    console.log(
      `timeline for 3 auditors: ${medianDays3} days - ${skewedDays3} days`,
    );
    console.log(
      `cost for 3 auditors budget: ${cost3Low * 3} - ${cost3High * 3}`,
    );
