...
export class TestController {
   ...
   */
  @Post('shared-volume')
  async testSharedVolume(
    @Body() request: TestSharedVolumeRequest,
  ): Promise<TestSharedVolumeResponse> {
    try {
      this.logger.log(
        `Testing shared volume communication with data: "${request.testData}"`,
      );
      ...
      this.logger.log(
        `✅ Step 1: Test data written to shared volume with ID: ${testId}`,
      );

      // Step 2: Call Rust service to read and verify the data
      const rustResponse = await this.testService.callRustTestEndpoint(
        testId,
        request.testData,
      );
