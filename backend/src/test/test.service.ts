...
export class TestService {
  ...
  constructor() {
    ...
    this.sharedVolumePath =
      process.env.SHARED_WORKSPACE_PATH || '/tmp/shared/workspaces';
    this.directMode =
      ['1', 'true', 'yes'].includes(
        (process.env.DIRECT_MODE || '').toLowerCase(),
      ) ||
      ['1', 'true', 'yes'].includes(
        (process.env.NO_SHARED_VOLUME || '').toLowerCase(),
      );
  }
  ...
  async writeTestData(testId: string, testData: string): Promise<void> {
    if (this.directMode) {
      this.logger.debug(
        'Direct mode enabled: skipping writeTestData filesystem operation',
      );
      return;
    }
    try {
   ...
   */
  async callRustTestEndpoint(
    testId: string,
    expectedData: string,
  ): Promise<RustTestResponse> {
    try {
      ...
      console.log(
        '🚀 ~ TestService ~ callRustTestEndpoint ~ this.rustServiceUrl:',
        this.rustServiceUrl,
      );
      const response = await axios.post(
        `http://${this.rustServiceUrl}${endpoint}`,
        payload,
        {
          timeout: 10000,
        },
      ...
      this.lastRustError = error?.message || String(error);

      if (axios.isAxiosError(error)) {
        if (error.response) {
          // Server responded with error status
          throw new Error(
            `Rust service error: ${error.response.status} - ${JSON.stringify(error.response.data)}`,
          );
  ...
  async cleanupTestData(testId: string): Promise<void> {
    if (this.directMode) {
      this.logger.debug(
        'Direct mode enabled: skipping cleanupTestData filesystem operation',
      );
      return;
    }
    try {
