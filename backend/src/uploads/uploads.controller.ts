...
export class UploadsController {
    ...
    FileInterceptor('file', {
      ...
      fileFilter: (req, file, cb) => {
        ...
        if (
          ...
          cb(
            new Error(
              'Invalid file type. Only zip, tar.gz, rar, and 7z files are allowed.',
            ),
            false,
          );
        }
  ...
  @ApiResponse({
    status: 201,
    description: 'The files have been successfully discovered.',
  })
  ...
  async discoverUploadedFiles(
    ...
    try {
      ...
      this.logger.log(
        `Discovering files in uploaded archive: ${file.originalname}`,
      );
