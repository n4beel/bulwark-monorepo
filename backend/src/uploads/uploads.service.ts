...
export class UploadsService {
  ...
  async discoverFilesInUpload(
    ...
    this.logger.log(
      `Discovering files in uploaded archive: ${originalFilename}`,
    );
  ...
  private async extractFile(
    ...
    try {
      if (fileExt === '.zip') {
        ...
        originalFilename.toLowerCase().includes('.tar.gz') ||
        fileExt === '.tar'
      ) {
      ...
      this.logger.log(
        `Successfully extracted ${originalFilename} to ${extractedPath}`,
      );
  ...
  private async extractRar(
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Failed to extract RAR file: ${error.message}. Make sure unrar or rar is installed.`,
        );
      }
    }
  }
  ...
  private async extract7z(
    ...
    try {
      ...
      throw new Error(
        `Failed to extract 7Z file: ${error.message}. Make sure 7z is installed.`,
      );
    }
  }
  ...
  private detectLanguageFromFiles(extractedPath: string): string {
    try {
      ...
      if (extCounts['.js'] > 0 || extCounts['.ts'] > 0)
  ...
  private getAllFiles(dirPath: string): string[] {
    ...
    const scanDirectory = (currentPath: string) => {
      try {
        ...
        this.logger.warn(
          `Error scanning directory ${currentPath}: ${error.message}`,
        );
      }
    };
    ...
    const buildStructureRecursive = (
      ...
      try {
        ...
        for (const item of dirItems) {
          ...
          if (stat.isDirectory()) {
            fileInfo.contents = buildStructureRecursive(
              fullPath,
              itemRelativePath,
            );
          }
    ...
    const findTestFiles = (items: any[]) => {
      for (const item of items) {
        if (item.type === 'file') {
          ...
          if (
            item.name.toLowerCase() === 'tests' ||
            item.name.toLowerCase() === 'test'
          ) {
