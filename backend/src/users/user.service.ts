...
export class UserService {
  ...
  async findOrCreateUser(githubUser: GitHubUser): Promise<UserDocument> {
    try {
      ...
      let user = await this.userModel
        .findOne({ githubId: githubUser.id })
        .exec();
      ...
      if (!user) {
        ...
        await user.save();
        this.logger.log(
          `Created new user: ${githubUser.login} (${githubUser.id})`,
        );
      } else {
  ...
  async findOrCreateGoogleUser(googleUser: GoogleUser): Promise<UserDocument> {
    try {
      // Try to find existing user
      let user = await this.userModel
        .findOne({ googleId: googleUser.id })
        .exec();
      ...
      if (!user) {
        ...
        await user.save();
        this.logger.log(
          `Created new user with Google: ${googleUser.email} (${googleUser.id})`,
        );
      } else {
      ...
      this.logger.error(
        `Failed to find or create Google user: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async linkGitHubAccount(
    ...
    try {
      ...
      const existingGithubUser = await this.userModel
        .findOne({ githubId: githubUser.id })
        .exec();

      if (existingGithubUser && String(existingGithubUser._id) !== userId) {
        throw new BadRequestException(
          'This GitHub account is already associated with another account',
        );
      }
      ...
      await user.save();
      this.logger.log(
        `Linked GitHub account ${githubUser.login} to user ${userId}`,
      );
  ...
  async linkGoogleAccount(
    ...
    try {
      ...
      await user.save();
      this.logger.log(
        `Linked Google account ${googleUser.email} to user ${userId}`,
      );
  ...
  async mergeAccounts(
    ...
    try {
      ...
      const secondaryUser = await this.userModel
        .findById(secondaryUserId)
        .exec();
      ...
      if (!primaryUser.email && secondaryData.email)
        primaryUser.email = secondaryData.email;
      // Merge emails array
      ...
      if (!primaryUser.name && secondaryData.name)
        primaryUser.name = secondaryData.name;
      if (!primaryUser.avatarUrl && secondaryData.avatarUrl)
        primaryUser.avatarUrl = secondaryData.avatarUrl;
  ...
  async generateToken(user: UserDocument): Promise<string> {
    ...
    if (this.whitelistService) {
      try {
        ...
        this.logger.warn(
          `Failed to check whitelist status for user ${user._id}: ${error.message}`,
        );
