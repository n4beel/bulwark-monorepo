...
import { staticAnalysisApi, FactorsResponse, FactorInfo as APIFactorInfo } from '@/services/api';
...
export default function ExportModal({ reports, onClose }: ExportModalProps) {
    ...
    const organizeFactorsIntoGroups = (factorsResponse: FactorsResponse) => {
        ...
        Object.entries(factorsResponse).forEach(([groupKey, groupData]) => {
            const factors = Object.entries(groupData.factors).map(([factorKey, factorInfo]) => {
    ...
    const handleExport = async () => {
        ...
        try {
            const reportIds = selectedReports.size > 0
                ? Array.from(selectedReports).filter(id => id != null && id !== undefined)
                : undefined;
            const factors = selectedFactors.size > 0 ? Array.from(selectedFactors) : undefined;

            // Only pass reportIds if we have valid IDs
            const validReportIds = reportIds && reportIds.length > 0 ? reportIds : undefined;

            const { blob, filename } = await staticAnalysisApi.exportReportsCSV(validReportIds, factors);
    ...
    const filteredFactorGroups = factorGroups.map(group => ({
        ...group,
        factors: group.factors.filter(factor =>
            factor.key.toLowerCase().includes(factorSearchTerm.toLowerCase()) ||
            factor.info.name.toLowerCase().includes(factorSearchTerm.toLowerCase()) ||
            factor.info.description.toLowerCase().includes(factorSearchTerm.toLowerCase())
        )
                ...
                <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-hidden">
                    ...
                    <div className="flex items-center justify-between p-6 border-b border-gray-200">
                        <div className="flex items-center">
                            ...
                            <div>
                                <h2 className="text-xl font-semibold text-gray-900">Export Analysis Reports</h2>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1">
                                                    <div className="font-medium text-gray-900">{report.repository}</div>
                                                    <div className="text-sm text-gray-600">
                                                        {report.language} • {report.framework} •
                                                        {new Date(report.createdAt.$date).toLocaleDateString()}
                                                    </div>
                                                </div>
                                            </div>
                                        );
                            ...
                            <div className="p-4 border-b border-gray-200">
                                <div className="flex items-center justify-between mb-3">
                                    <h3 className="text-lg font-medium text-gray-900 flex items-center">
                                        ...
                                        Select Data Fields ({selectedFactors.size}/{availableFactors.length})
                                    </h3>
                                    <div className="flex gap-2">
                                        <button
                            ...
                            <div className="flex-1 overflow-y-auto p-4">
                                {isLoadingFactors ? (
                                    <div className="flex items-center justify-center py-8">
                                        ...
                                        <span className="text-gray-600">Loading data fields...</span>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        {filteredFactorGroups.map((group, groupIndex) => (
                                            <div key={group.name} className="border border-gray-200 rounded-md">
                                                <button
                                                ...
                                                >
                                                    <div className="text-left">
                                                        ...
                                                        <div className="text-xs text-gray-500 font-normal">{group.description}</div>
                                                    </div>
                                                    <span className="text-sm text-gray-500">
                                                        {group.factors.filter(f => selectedFactors.has(f.key)).length}/{group.factors.length}
                                                    </span>
                                                </button>
                                                {group.expanded && (
                                                    <div className="p-2 space-y-1">
                                                        {group.factors.map((factor) => (
                                                            <div
                                                                key={factor.key}
                                                                onClick={() => toggleFactorSelection(factor.key)}
