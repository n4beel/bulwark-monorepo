...
describe("BulwarkStorage - Simple Storage Test (Arcium Encryption)", () => {
  ...
  it("can store audit data with Arcium-encrypted commit hash on devnet!", async () => {
    ...
    const auditData = {
      ...
      min_resources: 2,
      max_resources: 4,
      min_cost: BigInt(50000), // $500 in cents (minimum)
      max_cost: BigInt(100000), // $1000 in cents (maximum)
    ...
    console.log("  Timestamp:", new Date(Number(auditData.timestamp) * 1000).toISOString());
    ...
    console.log("  Cost Range: $" + (Number(auditData.min_cost) / 100).toFixed(2) + " - $" + (Number(auditData.max_cost) / 100).toFixed(2));
    ...
    const commitHash = "abc123def456789012345678901234567890abcdef1234567890abcdef123456";
    console.log("\n🔒 Step 4: PRIVATE Data (encrypting with Arcium):");
    console.log("  Original commit hash:", commitHash);

    // Convert commit hash to BigInt for Arcium encryption
    ...
    try {
      const storeSig = await program.methods
        .storeAuditResults(
          ...
          new anchor.BN(auditData.timestamp.toString()),
          auditData.min_days,
          auditData.max_days,
          auditData.min_resources,
          auditData.max_resources,
          new anchor.BN(auditData.min_cost.toString()),
          new anchor.BN(auditData.max_cost.toString()),
          auditData.score,
      ...
      console.log("-".repeat(70));
      console.log("\n📄 Transaction:");
      console.log(`   https://solscan.io/tx/${storeSig}?cluster=devnet`);
      console.log("\n📦 Audit Record Account:");
      console.log(`   https://solscan.io/account/${auditRecordPDA.toString()}?cluster=devnet`);
      console.log("\n🏛️  Bulwark Storage Program:");
      console.log(`   https://solscan.io/account/${program.programId.toString()}?cluster=devnet`);
      console.log("-".repeat(70));
      ...
      console.log("  Timestamp:", new Date(storedRecord.timestamp.toNumber() * 1000).toISOString());
      ...
      console.log("  Cost Range: $" + (storedRecord.minCost.toNumber() / 100).toFixed(2) + " - $" + (storedRecord.maxCost.toNumber() / 100).toFixed(2));
      ...
      expect(storedRecord.timestamp.toString()).to.equal(auditData.timestamp.toString());
      ...
      expect(storedRecord.minCost.toString()).to.equal(auditData.min_cost.toString());
      expect(storedRecord.maxCost.toString()).to.equal(auditData.max_cost.toString());
      ...
      const decryptedCommitHash = decryptedCommitHashBigInt.toString(16).padStart(64, '0');
